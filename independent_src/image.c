...
void reverse_filter(void)
{
    ...
    assert(image_info.color_type == 2 || image_info.color_type == 6);
    ...
    unsigned char *prev_scanline = malloc(sizeof(char) * image_info.width * byteDepth);
    for (int i = 0; i < image_info.width; ++i)
        prev_scanline[i] = 0;

    unsigned char *final_data = malloc(sizeof(char) * image_info.width * image_info.height * byteDepth);
